...
erDiagram
    ...
    education_sector_benchmarks {
        integer benchmark_year PK
        varchar sector PK
        double student_enrolment_share
        bigint student_enrolments
        bigint total_student_enrolments
...
- `wikidata_id` (`VARCHAR UNIQUE`): Canonical bare Wikidata Q-identifier (e.g. `Q4772000`), linked via APH ID property ([P10020](https://www.wikidata.org/wiki/Property:P10020)). Full entity URLs are normalized to bare QIDs.
...
- `boundary_id` (`VARCHAR PRIMARY KEY`): Deterministic identifier (e.g. `2025-clark-tas`).
- `election_year` (`INTEGER NOT NULL`): Federal election year the boundaries applied to (e.g. `2025`).
- `electorate` (`VARCHAR NOT NULL`): Official division name (e.g. `Clark`, `McMahon`, `O'Connor`).
- `state_or_territory` (`VARCHAR NOT NULL`): State or territory abbreviation (`ACT`, `NSW`, `NT`, `Qld`, `SA`, `Tas`, `Vic`, `WA`).
- `geometry` (`GEOMETRY NOT NULL`): Boundary polygon or multipolygon.
- `source_url` (`VARCHAR NOT NULL`): Direct download URL for the boundary shapefile archive.
- `source_dataset` (`VARCHAR NOT NULL`): Full descriptive title of the upstream dataset.
- `retrieved_at` (`TIMESTAMPTZ NOT NULL`): Ingestion or source currency timestamp.

### `education_sector_benchmarks`
Statistical reference benchmarks published by the Australian Bureau of Statistics (e.g. ABS Schools).
- `benchmark_year` (`INTEGER NOT NULL`): Annual statistical reference year (e.g. `2025`).
- `sector` (`VARCHAR NOT NULL CHECK (sector IN ('Government', 'Catholic', 'Independent'))`): Educational sector.
- `student_enrolment_share` (`DOUBLE NOT NULL`): Proportion of national student enrolments (0.628, 0.200, 0.172).
- `student_enrolments` (`BIGINT NOT NULL`): Total full-time/part-time student enrolments in the sector.
- `total_student_enrolments` (`BIGINT NOT NULL`): Total student enrolments across all sectors (denominator: 4,160,918 for 2025).
- `source_title` (`VARCHAR NOT NULL`): Formal statistical publication title (`Schools, 2025`).
- `source_url` (`VARCHAR NOT NULL`): Upstream ABS publication URL.
...
- `wikidata_id`: Resolved Wikidata Q-identifier (if matched).
- `field`: Field evaluated (`date_of_birth`, `gender`, or `wikidata_id`).
- `aph_value`: Value recorded in official APH biography.
- `wikidata_value`: Corresponding value extracted from Wikidata claims.
...
- `wikidata_id`: Wikidata Q-identifier for the institution.
- `wikipedia_url`: English Wikipedia sitelink.
- `country`: Country of location (e.g. `United Kingdom`, `New Zealand`).
- `locality`: Suburb, city, or administrative region.
- `latitude`, `longitude`: Geographic coordinates where present on Wikipedia/Wikidata.
...
│   └── wikimedia/               # Raw cached Wikimedia JSON responses
│       ├── members/             # Cached member SPARQL queries ({aph_id}.json)
│       └── institutions/        # Cached Wikipedia pageprops and claims
├── processed/                   # Canonical output artifacts generated by CLI
│   ├── *.parquet                # Canonical Parquet table exports
│   │   ├── electoral_boundaries.parquet       # Valid GeoParquet with WKB & bbox
│   │   ├── education_sector_benchmarks.parquet# Statistical benchmark shares
│   │   └── ...                  # Member, institution, and service tables
...
│   ├── wikimedia_member_review.csv  # Member demographic QA & discrepancy review
│   └── wikimedia_school_review.csv  # Wikimedia unmatched school suggestions
